...
impl DeadlineTable for FipsTable {
    fn name(&self) -> &'static str { "FIPS 140-3" }

    fn deadline_for(&self, alg: &AlgorithmId) -> Option<DeadlineInfo> {
        Some(match alg {
            // Not FIPS approved
            AlgorithmId::ChaCha20Poly1305 =>
                DeadlineInfo { deprecated_year: 2000, disallowed_year: 2000, note: "Not FIPS 140-3 approved" },
            ...
            | AlgorithmId::Ed25519
            | AlgorithmId::Dh { .. } =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2030, note: "Transitioning to PQC" },
            _ => return None,
        })
    }
