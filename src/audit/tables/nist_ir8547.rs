...
impl DeadlineTable for NistIr8547Table {
    ...
    fn deadline_for(&self, alg: &AlgorithmId) -> Option<DeadlineInfo> {
        Some(match alg {
            AlgorithmId::Rsa { min_bits } if *min_bits < 2048 =>
                DeadlineInfo { deprecated_year: 2000, disallowed_year: 2000, note: "Already disallowed" },
            AlgorithmId::Rsa { min_bits } if *min_bits <= 3072 =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2030, note: "Primary hard wall" },
            AlgorithmId::Rsa { .. } =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2035, note: "Extended deadline for RSA-4096" },
            AlgorithmId::EcP256 | AlgorithmId::EcP384 =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2030, note: "" },
            AlgorithmId::EcP521 =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2035, note: "Extended deadline" },
            AlgorithmId::X25519 | AlgorithmId::Ed25519 =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2030, note: "" },
            AlgorithmId::X448 | AlgorithmId::Ed448 =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2035, note: "Extended deadline" },
            AlgorithmId::Dh { min_bits } if *min_bits < 2048 =>
                DeadlineInfo { deprecated_year: 2000, disallowed_year: 2000, note: "Already disallowed" },
            AlgorithmId::Dh { min_bits } if *min_bits <= 2048 =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2030, note: "" },
            AlgorithmId::Dh { .. } =>
                DeadlineInfo { deprecated_year: 2024, disallowed_year: 2035, note: "Extended deadline for DH-3072+" },
            _ => return None,
        })
    }
...
mod tests {
    ...
    fn rsa2048_disallowed_2030() {
        ...
        let info = table.deadline_for(&AlgorithmId::Rsa { min_bits: 2048 })
    ...
    fn p521_disallowed_2035() {
        let table = NistIr8547Table;
        let info = table.deadline_for(&AlgorithmId::EcP521)
