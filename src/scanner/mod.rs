...
async fn scan_single(target: String, config: &ScanConfig) -> TargetReport {
    ...
    match pqc_result {
        ...
        Ok(pqc) => {
            ...
            let findings = generate_findings(&probe_results, model.as_ref());
...
fn generate_findings(probe: &ProbeResults, model: &dyn crate::audit::scoring::model::ScoringModel) -> Vec<Finding> {
    ...
    if let Ok(pqc) = &probe.pqc_handshake {
        ...
        // Cert chain findings (generated by audit_chain, merged here)
        // The cert_chain.findings are already severity-stamped by audit_chain using NistIr8547Table.
        // We re-stamp them with the active model's severity for consistency.
        let cert_report = audit_chain(&pqc.cert_chain_der);
        for f in cert_report.findings {
...
mod tests {
    ...
    fn classical_group_generates_finding() {
        ...
        let findings = generate_findings(&probe, model.as_ref());
        assert!(
